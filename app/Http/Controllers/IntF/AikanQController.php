...
class AikanQController extends Controller
{
    ...
    protected function getLiveMatches($sport, $bet = '') {
        ...
        $ch_time = date('Y-m-d H:i:s', strtotime('-7 days'));
        ...
        $ch_table = DB::raw('(SELECT live_id FROM match_live_channels WHERE isPrivate = ' . MatchLiveChannel::kIsPrivate . ' AND created_at >= \'' . $ch_time . '\' AND `show` = ' . MatchLiveChannel::kShow . ' GROUP BY live_id) as match_live_channels');
