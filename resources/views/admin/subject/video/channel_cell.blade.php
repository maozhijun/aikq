<div style="margin-left: 10px;padding-bottom: 10px;">
    ...
    <button type="button" class="btn btn-success" type="button" onclick="saveVideoChannel(this, '{{$channel->id or ''}}');">保存</button>
    <button type="button" class="btn btn-danger" type="button" onclick="delVideoChannel(this, '{{$channel->id or ''}}');">删除</button>
    @if(isset($channel))
        <?php
            $sv_id = $channel->sv_id;
            if (strlen($sv_id) >= 4) {
